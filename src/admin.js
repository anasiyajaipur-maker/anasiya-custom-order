...
function renderOrders() {
  $(`#orders-list`).innerHTML = state.orders.length ? state.orders.map((o)=>`<article class="order-item"><div class="order-top"><strong>${esc(o.customerName || `Custom order`)}</strong><span class="status-pill">${esc(o.status || `new`)}</span></div><div class="order-row"><span>Style</span><strong>${esc(o.productName)}</strong><span>Fabric</span><strong>${esc(o.fabricName)}</strong></div></article>`).join(``) : `<div class="empty-card"><p>No custom orders yet.</p></div>`;
}
